...
bool ULayoutGeneratorWFC::GenerateInternal(ALayout* Layout, int32 Seed)
{
	...
	while (CellPossibilities.Num() > 0)
	{
		...
		// If we find a cell with no possible rows we've hit a contradiction, try to redo the last cell to solve it OR abort if its unsolveable...
		if (CellPossibilities[CurrentCellKey].Num() <= 0)
		{
			if (SolveContradictionBannedPossibilities.Num() < DataTableMap.Num() * 4)
			{
				...
				Layout->GetCell(LastCellKey)->SetRowName("None", 0);
